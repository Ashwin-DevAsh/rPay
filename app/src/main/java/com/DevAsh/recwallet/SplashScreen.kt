                    ...
                    AndroidNetworking.get(ApiContext.apiUrl + ApiContext.paymentPort + "/getMyState?number=${DetailsContext.phoneNumber}")
                        ...
                        .getAsJSONObject(object: JSONObjectRequestListener {
                            override fun onResponse(response: JSONObject?) {
                                ...
                                AndroidNetworking.get(ApiContext.apiUrl + ApiContext.paymentPort + "/getTransactions?number=${DetailsContext.phoneNumber}")
                                    .addHeaders("jwtToken",DetailsContext.token)
                                    .setPriority(Priority.IMMEDIATE)
                                    .build()
                                        ...
                                        override fun onResponse(response: JSONArray?) {
                                            ...
                                            for(i in 0 until response!!.length()){
                                                transactions.add(Transaction(
                                                   ...
                                                   amount = response.getJSONObject(i)["Amount"].toString(),
                                                   time = dateToString(response.getJSONObject(i)["TransactionTime"].toString()),
                                                   type = if (response.getJSONObject(i)["From"]==DetailsContext.phoneNumber)
                                                              "Send"
                                                          else "Received"
                                                ))
                                            }
